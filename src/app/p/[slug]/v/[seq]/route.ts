...
export async function GET(_req: Request, ctx: { params: Promise<{ slug: string; seq: string }> }) {
  ...
  const row = db
    .prepare(
      `SELECT v.html, p.platform, p.connectors FROM projects p
    ...
    .get(seqNum, slug) as { html: string; platform: "web" | "mobile"; connectors: string | null } | undefined;

  if (!row) return notFoundApp();
  return serveAppHtml(row.html, slug, { platform: row.platform, snapshot: true, connectors: row.connectors });
}
