...
- **⧗ 任务中心**:侧边栏「任务」入口带进行中徽标,侧滑面板列出全部排队/进行中任务
  (阶段进度、排队位次、已用时长,点开直达工作台)与最近 24h 完成记录;Dashboard
  项目卡同步显示 ⚙ 构建中 / ⏳ 排队中
- **⇉ 后台构建**:提示卡一键"发射后不管"——服务端直接开跑,留在原页连发下一个;
  默认提交也已服务端化(进工作台即接上实时流)
- **按套餐分级并发**:同时构建 免费 1 / 专业 2 / 旗舰 3,10 分钟生成限频 3/6/9 次,
  全局执行并发 3;超出并发即时 429 并引导升级(不烧限频、不扣积分)
...
生成的应用不许直连外网,一切外部数据经平台白名单代理 `/api/connectors/[name]`
(参数校验 + 缓存 + 限流 + CORS),启用后自动向 Engineer 注入用法文档、向应用注入
`window.quark.connectors` 客户端:

| 连接器 | 凭证 | 可用范围 |
|---|---|---|
| 🌦 天气 · 💱 汇率 · 🔲 二维码 · 📁 Google Drive(公开文件/文档/表格) | 免凭证 | 预览与发布均可用 |
| 🐙 GitHub(仓库/Issues/文件) · 🎨 Figma(文件结构/节点渲染) · 📓 Notion(查库/读页) | 用户自填只读令牌 🔑 | **仅属主工作台预览** |
| 💬 Slack(发消息,≤500 字 · 5 条/分) | Incoming Webhook 🔑 | **仅属主工作台预览** |

- 凭证在 **设置 → 连接器凭证** 粘贴保存(服务端仅存本地 SQLite,任何 API 只回掩码);
  零 OAuth 应用注册,当天可用
- 预览鉴权:沙箱 iframe 是 opaque origin(无 cookie),平台向属主预览注入 **30 分钟
  短时令牌**,连接器 API 校验令牌 → 取属主凭证 → 白名单代理调用;发布/导出产物不带
  令牌,凭证连接器自动变为明确报错的桩(免凭证连接器照常工作)
- Slack Webhook 存入与调用双重校验域名(SSRF 防线);凭证类响应 `private, no-store`,
  缓存按用户隔离
...
- **🎙 语音输入**(首页与工作台,v20 重构):点击录音、再点停止(最长 60 秒),浏览器
  MediaRecorder 采集 → 平台代理 → **自托管 faster-whisper** 中文转写(实测 5 秒音频约 0.7 秒
  出词)。全部现代浏览器可用,不依赖任何第三方语音云;录音仅转写即弃、不落盘。
  (v11 曾用 Web Speech API,其识别依赖 Google 语音服务,在国内网络不可用——已整体替换,如实说明)
...
## Fusion Cloud 与 AI 钱包(v21)

**把 214 模拟成云主机**:工作台「⚡ 云实例」把已发布制品部署上云——

- 工程引擎应用**在云上真实构建**(源码上云 → npm/vite 构建,构建日志可查),单文件应用产物直传;
  云端常驻托管,平台 `/c/{slug}` 公网访问;实例可 停止/启动/删除
- **按量计费**:部署 2 积分 + 运行 1 积分/小时,平台自动按整小时结算,**余额不足自动停机**——
  积分从"扣费开关"升级为驱动云资源的计量货币
- **💰 AI 钱包 `/wallet`**(用户菜单进入):余额与续航估算、消费构成(生成/打包/云服务/充值)、
  完整流水、兑换码、运行实例一键停止
- 边界如实:云实例是共享静态托管(模拟云定位,非独立 VM);工程引擎云产物为原生 vite 构建,
  不含 quark.storage 注入(应用自动回退 localStorage)
